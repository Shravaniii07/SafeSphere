...
function HeartbeatLine() {
  return (
    <svg viewBox="0 0 200 40" className="w-48 h-8 text-accent/30">
    ...
    <>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[65vh] gap-10 stagger-children">
        <div className="text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-red-50 border border-red-100/80 rounded-xl text-accent text-xs font-display font-semibold mb-5">
          ...
          <p className="text-slate-400 max-w-sm mx-auto text-sm">Press the button to immediately alert your emergency contacts with your location</p>
        </div>

        {/* SOS Button */}
        <div className="relative flex items-center justify-center">
          <div className={`absolute w-[260px] h-[260px] rounded-full border-2 border-accent/25 opacity-0 ${isCountingDown ? 'animate-sos-ring-fast' : 'animate-sos-ring'}`} />
          <div className={`absolute w-[260px] h-[260px] rounded-full border-2 border-accent/15 opacity-0 ${isCountingDown ? 'animate-sos-ring-fast' : 'animate-sos-ring'} animate-sos-ring-2`} />
          <div className={`absolute w-[260px] h-[260px] rounded-full border-2 border-accent/8 opacity-0 ${isCountingDown ? 'animate-sos-ring-fast' : 'animate-sos-ring'} animate-sos-ring-3`} />
          <button
            onClick={isCountingDown ? cancelCountdown : startCountdown}
            className={`relative z-10 w-[200px] h-[200px] rounded-full text-white flex flex-col items-center justify-center gap-3 shadow-glow-accent cursor-pointer transition-all duration-300 ${isCountingDown ? 'scale-110 shadow-[0_0_80px_rgba(239,68,68,0.45)]' : 'hover:scale-[1.04] hover:shadow-[0_0_60px_rgba(239,68,68,0.35)] active:scale-[0.96]'}`}
        ...
        {isCountingDown ? (
          ...
          <div className="flex gap-4 flex-wrap justify-center">
            <Button variant="outline" size="lg" onClick={() => { window.location.href = 'tel:100'; toast.success('Calling emergency contacts...') }}>
              <Phone className="w-[18px] h-[18px]" /> Call Contacts
            </Button>
            <Button variant="outline" size="lg" onClick={() => {
              ...
              <MapPin className="w-[18px] h-[18px]" /> Send Location
            </Button>
          </div>
        )}
